...
const C = {
  roseDk:"#2D5A4F",rose:"#4A7C6F",roseLt:"#E8F5F2",
...
body{font-family:'Nunito',sans-serif;background:#2D5A4F;display:flex;justify-content:center;min-height:100vh;}
...
.ni{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;padding:6px 0;}
.ni-i{font-size:24px;}
.ni-l{font-size:9px;color:#6b6760;}
.ni.on .ni-l{color:#2D5A4F;font-weight:600;}
.ni-d{width:4px;height:4px;border-radius:50%;background:#4A7C6F;display:none;margin:0 auto;}
...
.tag.rz{background:#E8F5F2;border-color:#4A7C6F;color:#2D5A4F;}
.tag.bz{background:#E6F1FB;border-color:#378ADD;color:#0C447C;}
.pbtn{background:#2D5A4F;color:white;border-radius:12px;padding:13px;width:100%;font-size:14px;font-weight:600;transition:opacity .15s;}
...
@keyframes slideUp{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}} @keyframes bo{0%,80%,100%{transform:scale(.6);opacity:.4}40%{transform:scale(1);opacity:1}}
...
.ta{width:100%;background:#FAF8F5;border:.5px solid #e0dbd4;border-radius:10px;padding:11px 13px;font-size:15px;color:#2C2A28;resize:none;line-height:1.6;}
`;
  ...
  return (
    <div className="screen" style={{ background: C.roseDk }}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 0 20px" }}>
        <div style={{ textAlign: "center" }}><div style={{ fontFamily: "'Playfair Display',serif", fontSize: 56, color: "white", fontWeight: 600, letterSpacing: "-1px" }}>Harlo</div><div style={{ fontSize: 12, color: "rgba(255,255,255,.6)", letterSpacing: ".2em", textTransform: "uppercase", marginTop: 10, animation: "slideUp .8s ease .3s both" }}>For every high and every low</div></div>
...
function EntryScreen({ go, user, onSave }) {
  ...
  useEffect(() => {
    ...
    supabase.from("medication_logs").select("taken").eq("user_id", user.id).eq("log_date", d).single().then(({ data }) => { if (data) setMedTaken(data.taken); });
  }, [user.id]);
  ...
  const allTags = [
    ...
    { l:"hopeful", e:"🌱", s:"rz" }, { l:"proud", e:"✨", s:"rz" }, { l:"frustrated", e:"😤", s:"rz" }, { l:"loved", e:"🥰", s:"rz" }
  ];
  ...
  return (
    <div className="screen">
      <div className="hdr" style={{ background: C.roseDk }}>
        <div className="back" onClick={() => go("home")}></div>
      ...
      <div className="body">
        ...
        <div onClick={async () => { const next = medTaken !== true; setMedTaken(next); await supabase.from("medication_logs").upsert({ user_id: user.id, log_date: getLocalDate(), taken: next }, { onConflict: "user_id,log_date" }); }} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: medTaken ? "#E8F5F2" : "#F5F5F5", borderRadius: 12, padding: "12px 14px", marginBottom: 12, cursor: "pointer" }}><div style={{ display: "flex", alignItems: "center", gap: 10 }}><span style={{ fontSize: 20 }}>💊</span><div><div style={{ fontSize: 14, fontWeight: 600, color: C.ink }}>Medication today?</div><div style={{ fontSize: 12, color: C.mid, marginTop: 1 }}>{medTaken ? "Taken" : "Tap to log"}</div></div></div><div style={{ width: 26, height: 26, borderRadius: "50%", background: medTaken ? C.teal : "#e0dbd4", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, color: "white" }}>{medTaken ? "✓" : ""}</div></div><div className="slbl">How are you feeling?</div>
          ...
          <>
            ...
            <div style={{ background: C.tealLt, borderRadius: 12, padding: "12px 14px", borderLeft: `3px solid ${C.teal}`, borderTopLeftRadius: 0, marginBottom: 8 }}>
              <div style={{ fontSize: 9, textTransform: "uppercase", color: C.tealDk, fontWeight: 600, marginBottom: 5, display: "flex", alignItems: "center", gap: 5 }}>
                <div style={{ width: 6, height: 6, borderRadius: "50%", background: C.teal }}></div>Harlo replied
              </div>
              {currentAiReply
            ...
            <div className="card">
              ...
              {ctab === "self" ? (
                ...
                <>
                  <div style={{ fontSize: 10, color: C.mid, fontStyle: "italic", marginBottom: 8 }}>Ask Harlo a follow-up question.</div>
...
function CheersScreen({ go, user, cheers, setCheers }) {
  ...
  const cheerCard = (c, isSent) => (
    <div key={c.id} className="card" style={{ borderColor: (!isSent && !c.is_read) ? "#F4C0D1" : "#e8e2da" }} onClick={() => !isSent && markRead(c.id)}>
      ...
      {c.type === "voice" && <div style={{ background: C.purpleLt, borderRadius: 9, padding: "9px 12px" }}>{c.caption ? <audio controls src={c.caption} style={{ width: "100%" }} /> : <div style={{ display: "flex", alignItems: "center", gap: 10 }}><div style={{ width: 28, height: 28, borderRadius: "50%", background: C.roseDk, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, color: "white" }}>▶</div><div style={{ fontSize: 11, color: C.purpleDk, fontWeight: 600 }}>Voice note</div></div>}</div>}
      {!isSent && (
  ...
  return (
    <div className="screen">
      ...
      <div className="body">
        ...
        <button className="pbtn" style={{ marginTop: 14, background: C.roseDk }} onClick={save}>Save preferences</button>
...
function ShareToComm({ entry, currentAiReply, user, go }) {
  ...
  const buildContent = () => {
    ...
    if (inc.ai && currentAiReply) parts.push(`Harlo reflected: "${currentAiReply}"`);
      ...
      {open && (
        <div style={{ background: C.purpleLt, borderRadius: 12, padding: "12px 14px", marginTop: 8, border: `.5px solid ${C.purple}` }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 8, paddingTop: 8, borderTop: `.5px solid ${C.purple}` }}>
            ...
            <button onClick={share} disabled={sharing || !buildContent()} className="pbtn" style={{ width: "auto", padding: "8px 16px", background: C.roseDk, fontSize: 12 }}>
...
function CommunityScreen({ go, user }) {
  ...
  const loadFeed = async () => {
    ...
    let query = supabase.from("posts").select("*, profiles(username, display_name), post_likes(user_id)").order("created_at", { ascending: false }).limit(30);
          ...
          <>
            <div className="card" style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <button className="pbtn" onClick={submitPost} disabled={posting || !newPost.trim()} style={{ width: "auto", padding: "9px 18px", background: C.roseDk, fontSize: 12 }}>
              ...
              <div key={post.id} className="card" style={{ marginBottom: 10 }}>
                ...
                <div style={{ display: "flex", gap: 8, borderTop: ".5px solid #f0ebe4", paddingTop: 8 }}>
                  ...
                  {post.user_id !== user.id && (
                    <>
                      <button onClick={async () => {
                        if (window.confirm("Report this post? We will review it within 24 hours.")) {
                          await supabase.from("post_reports").insert({ post_id: post.id, reporter_id: user.id, reason: "User reported" });
                          alert("Thank you — we will review this post within 24 hours.");
                        }
                      }} className="tag" style={{ fontSize: 11, color: "#999" }}>
                        🚩 Report
                      </button>
                      <button onClick={async () => {
                        if (window.confirm("Block this user? Their posts will be removed from your feed immediately.")) {
                      ...
                      }} className="tag" style={{ fontSize: 11, color: "#999", marginLeft: "auto" }}>
                        🚫 Block
                      </button>
                    </>
                  )}
                ...
                {commentingOn === post.id && (
                  <div style={{ marginTop: 10 }}>
                    ...
                    <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
                      ...
                      <button onClick={() => submitComment(post.id)} className="pbtn" style={{ width: "auto", padding: "0 14px", background: C.roseDk, fontSize: 12 }} disabled={!commentText.trim()}>Send</button>
                    </div>
                  </div>
                )}
              </div>
          ...
          <>
            <div className="card" style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button onClick={createGroup} className="pbtn" style={{ width: "auto", padding: "0 14px", background: C.roseDk, fontSize: 12 }} disabled={creatingGroup || !newGroup.trim()}>
  ...
  return (
    <div className="screen">
      ...
      <div className="body">
        ...
        <button onClick={() => supabase.auth.signOut()} className="pbtn" style={{ background: C.roseDk, marginBottom: 10 }}>Sign out</button>
        {!showDelete ? (
...
function TermsScreen({ go }) {
  return (
    <div className="screen">
      ...
      <div className="body">
        {[
          { t: "Acceptance of terms", b: "By using Harlo, you agree to these terms. If you do not agree, please do not use the app." },
          { t: "Not a medical service", b: "Harlo is a wellness and journaling app, not a medical or mental health service. AI reflections are not a substitute for professional advice. If you are in crisis, please use the helplines in the app." },
    ...
    <div className="screen" style={{ background: C.roseDk }}>
      ...
      {step === 0 && (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 28px", textAlign: "center" }}>
          <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 52, color: "white", fontWeight: 600, lineHeight: 1.1, marginBottom: 16 }}>Harlo</div>
          <div style={{ fontSize: 14, color: "rgba(255,255,255,.65)", lineHeight: 1.7, marginBottom: 48 }}>For every high and every low.</div>
      ...
      {step === 2 && (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "24px 28px 40px" }}>
          ...
          <input value={username} onChange={e => { setUsername(e.target.value.replace(/\s/g,"_").replace(/[^a-zA-Z0-9_-]/g,"")); setUsernameErr(""); }} placeholder="your_username" style={{ width: "100%", padding: "14px 16px", borderRadius: 12, border: "none", background: "rgba(255,255,255,.15)", color: "white", fontSize: 16, marginBottom: 8, outline: "none" }} />
          <div style={{ fontSize: 11, color: "rgba(255,255,255,.4)", marginBottom: 16 }}>Letters, numbers, underscores and hyphens</div>
  ...
  return (
    <div className="screen" style={{ background: C.roseDk }}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 28px" }}>
        <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 32, color: "white", fontWeight: 600, marginBottom: 8 }}>New password</div>
        <div style={{ fontSize: 14, color: "rgba(255,255,255,.65)", marginBottom: 32 }}>Choose a new password for your account.</div>
        {err && <div className="err" style={{ width: "100%" }}>{err}</div>}
        {ok && <div className="ok" style={{ width: "100%" }}>Password updated! Signing you in...</div>}
        <input type="password" className="inp" value={pw} onChange={e => setPw(e.target.value)} placeholder="New password" style={{ marginBottom: 12 }} />
        <input type="password" className="inp" value={pw2} onChange={e => setPw2(e.target.value)} placeholder="Confirm new password" />
        <button className="pbtn" onClick={save} disabled={loading || ok} style={{ marginTop: 8 }}>{loading ? "Saving..." : "Set new password"}</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="screen" style={{ background:'#0d1117' }}>
      <div style={{ padding:'52px 20px 20px', background:'#0d1117' }}>
        <div style={{ fontSize:11, color:'rgba(255,255,255,.4)', marginBottom:6, cursor:'pointer' }} onClick={() => go('home')}>back</div>
        <div style={{ fontFamily:"'Playfair Display',serif", fontSize:26, color:'white', fontWeight:600 }}>Sleep Stories</div>
        <div style={{ fontSize:12, color:'rgba(255,255,255,.45)', marginTop:4 }}>Ambient sound coming soon</div>
      </div>
      <div style={{ padding:'0 16px 100px', flex:1, overflowY:'auto' }}>
        {stories.map(s => (
          <div key={s.id} style={{ background:'#1a1f2e', borderRadius:16, padding:'18px 16px', marginBottom:12, border:'1px solid rgba(255,255,255,.06)' }}>
  ...
  if (!splashDone) return (
    <>
      ...
      <div className="phone" style={{ alignItems: "center", justifyContent: "center", background: C.roseDk }}>
        <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 44, color: "white", fontWeight: 600, marginBottom: 24 }}>Harlo</div>
        <Spinner color="#F4C0D1" />
      </div>
    </>
  );

  if (isReset) return <><style>{css}</style><div className="phone"><ResetPasswordScreen onDone={() => setIsReset(false)} /></div></>;
